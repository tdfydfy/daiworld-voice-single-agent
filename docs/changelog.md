...
- 修复 `1.2.16` 附件回归：HarmonyOS 开始在 `session.create` 携带移动端提示词后，Adapter 不再跳过 Agent 预设，而是合并 Agent 预设、移动端偏好和不可覆盖的附件投递契约。Agent 发送图片或文件前必须复制到首个 `VOICE_ARTIFACT_ROOTS` 目录并输出独立 `MEDIA:<绝对路径>` 行；继续保持 `/tmp` 默认白名单，不扩大敏感路径访问。
- 发布 `1.2.16 (1020016)`：用户决定固定为当前发布基线。审批语音口令严格限定为整句“同意/取消”，只清理整句空格及口令两端 ASR 标点，近义词、句中标点、夹带文字、数字和序号均不触发；澄清选项不编号并等待完整收句，已知语义文本归一化回传，其他内容原样交给 Hermes。Node `91/91`、43 个 ETS 静态校验、ArkTS 类型检查和签名构建通过；HAP SHA-256 为 `39FE0D32F3E347160ADF562AE65376424BF2C88E681E6B2A25C02597824A0430`，已覆盖安装并成功拉起。
- 发布与真机验收：HarmonyOS `1.2.14 (1020014)` 完成精确“关闭话筒”“打开话筒”“停止任务”“退出软件”控制策略；“退出软件”在资源释放后结束 Ability，“停止任务”立即停止本地输出并准确区分远端确认、失败和待确认状态。用户确认本轮真机产品测试成功。
- 停止任务一致性：新增请求代次与 Session 隔离，防止重复停止、迟到回调、会话切换或新提问覆盖当前状态；C0 探针及帧级测试已就绪，但真实 Hermes `session.interrupt` 是否清空全部 `queued:true` 任务仍待受控环境取证。
- 验证与构建：Node `88/88`、Python `37/37`、42 个 ETS 静态校验、ArkTS 类型检查和签名 HAP 构建全部通过；C0 探针定向测试 `6/6` 通过。
- 发布与真机验收：`1.2.12` 的 Bluetooth SCO 短音频路径经真机验证不可用，提示音 PCM 会在 SCO 冷启动完成前被消费；`1.2.13 (1020013)` 改为通过 `VOICE_ASSISTANT/A2DP` 播放短提示音后，用户确认“收到”和“思考”提示音均已恢复正常。
- 发布 `1.2.11 (1020011)`：完成过程时间线与历史回放闭环。实时事件和 Hermes 历史消息共用活动 reducer，支持稳定活动 ID、状态更新、摘要/明细、活动级计时和单项展开；迟到或重复工具结果原地更新，不重复追加。
- 历史恢复：历史详情失败时正文仍可通过 `session.resume` 回退，过程区域明确显示“历史过程未完整载入”；工具参数和结果摘要对 token、password、secret、Bearer 等敏感值脱敏。
- 验证与构建：Node `71/71`、Python `36/36`、40 个 ETS 静态校验、ArkTS 类型检查和签名 HAP 构建全部通过。签名 HAP SHA-256 为 `AD0756E11C7915A5BA40B54D9ED27EF0DC67CDAFA45E6D95D54BD206D5579656`。
- 真机验收：`1.2.11` 已保留数据覆盖安装到设备 `6HQ0226409028766`，用户确认过程时间线 T6-1 实时过程、T6-2 历史回放、T6-3 不完整历史均通过；此前语音运行时 D1-D8 也已通过验收。
