...
- [x] [设置布局] 使用成组双列布局并强化分区
  ...
  - 实现结果：`Index.ets` 将连接、Agent 运行配置、语音引擎和应用设置分成独立面板；地址/口令、Provider/模型、音色/语速使用等宽双列 Row，控件行为和保存逻辑保持不变。ArkUI 未提供可在当前项目静态约束中安全使用的运行时断点 API，因此窄屏通过现有 Scroll 和等权布局保持可用，后续真机验收再决定是否需要单列回退。
  - 验证状态：Harmony 静态校验（28 个 ETS 文件）、ArkTS 类型检查、签名 HAP 构建、Node 19 项测试和 `git diff --check` 通过；真机视觉验收待后续进行。最新 HAP 为 `entry-default-signed.hap`，SHA-256 为 `6A2ADE800AECDD081A2486085F724F4D0D0F7BD04B439693C7E54310A3285F88`。
