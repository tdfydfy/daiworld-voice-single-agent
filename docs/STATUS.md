...
- 设置页开发：已完成分组双列布局，连接、Agent 运行配置、语音引擎和应用设置各自独立；地址/口令、Provider/模型、音色/语速成组并排显示。未改变设置保存、模型切换或语音逻辑，Harmony 静态校验、ArkTS 类型检查、签名 HAP 构建和 Node 回归通过，真机视觉验收待后续统一进行。最新 HAP SHA-256 为 `6A2ADE800AECDD081A2486085F724F4D0D0F7BD04B439693C7E54310A3285F88`。
- 真机冒烟：设备 `6HQ0226409028766` 已重新连接；最新 `1.2.1 (1020001)` 签名 HAP 已使用 `hdc install -r` 保留数据覆盖安装，包管理器确认版本码 `1020001`，`EntryAbility` 拉起成功，进程 `cc.daiworld.voice.singleagent` 存在并处于前台。未执行设置点击、语音输入、听感或视觉判断，等待用户手动验收双列布局。
- 用户验收：已确认设置页面分组与双列布局通过；该设置布局开发项关闭。当前没有新的明确代码开发阻塞，后续优先按验收顺序验证连续 ASR 自然停顿、耳机/音频闭环、审批与澄清，以及 Agent 切换等已实现能力。
