...
Implementation status: the speakerphone preference is persisted, defaults to `EARPIECE`, and switches the no-accessory fallback to `SPEAKER`; the connected-device path remains system-managed. Bounded AudioSession recovery is implemented but still awaits a true-device `audioSessionDeactivated` recovery event. Automatic TTS dispatch is implemented and verified through engine synthesis. The candidate focus fix removes the system-TTS communication-session lease and activates microphone-owned communication sessions with `CONCURRENCY_MIX_WITH_OTHERS`; static verification, ArkTS type checking, and HAP assembly pass. Audible playback with the microphone enabled and disabled still requires true-device acceptance, including confirmation that `6800301` is gone.
