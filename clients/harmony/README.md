...
The in-app help page explains when to use `关闭话筒`, `打开话筒`, `停止任务`, and `退出软件`. Settings also exposes a mobile-only system-instructions field. Its default asks for concise, conversational replies during voice interaction; saving it affects newly created HarmonyOS sessions only and does not modify or replace Hermes backend prompt configuration, Adapter safety policy, or the attachment-delivery contract.
